...
impl Modules {
    pub fn add_item_id(&mut self, tcx: TyCtxt, item_id: ItemId) {
        ...
        if let ItemKind::Mod(_, module) = item_kind {
            ...
            self.add_module(current, module, (v_parent.len() + 1) as u8, direct_parent);

            // Add parent modules: shallow to deep, starting from the next level of root.
            for (idx, &[child, parent]) in v_parent.array_windows().rev().enumerate() {
                let m = tcx.hir_get_module(child).0;
                self.add_module(child, m, (idx + 2) as u8, parent);
            }
        }
    }
    ...
    pub fn add_root(&mut self, tcx: TyCtxt) {
        ...
        self.add_module(root, module, 1, root);
    }

    fn add_module(&mut self, current: LocalModId, module: &Mod, level: u8, parent: LocalModId) {
        self.map.entry(current).or_insert_with(|| Module {
            level,
            parent,
            items: module.item_ids.iter().copied().collect(),
        });
    }
        ...
        self.map
            ...
            .map(|(key, val)| OutModule {
                ...
                items: val
                    .items
                    .iter()
                    .map(|item| def_path_str(item.owner_id.to_def_id(), tcx))
                    .sorted()
                    .collect(),
